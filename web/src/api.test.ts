...
describe('api contract', () => {
  ...
  it('sends the session bearer token and decodes typed JSON', async () => {
    ...
    const fetcher = vi.fn((_input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return Promise.resolve(new Response(JSON.stringify({ models: 2 }), { headers: { 'Content-Type': 'application/json' } }))
    })
    vi.stubGlobal('fetch', fetcher)
    await expect(api<{ models: number }>('/api/dashboard')).resolves.toEqual({ models: 2 })
  })

  it('exposes backend errors', async () => {
    vi.stubGlobal('fetch', vi.fn(() => Promise.resolve(new Response(JSON.stringify({ error: 'denied' }), { status: 401 }))))
    await expect(api('/api/models')).rejects.toEqual(expect.objectContaining<Partial<APIError>>({ status: 401, message: 'denied' }))
  })
})
...
describe('model workflow',()=>{it('sends only the registered model identity for downloads',()=>{const model={id:'model-id',name:'model',kind:'huggingface' as const,source:'org/model',repository:'org/model',revision:'main',local_path:'',status:'registered',size_bytes:0,created_at:'',updated_at:''};expect(linkedDownload(model,'job-id','secret')).toEqual({id:'job-id',model_id:'model-id',token:'secret'})})})
