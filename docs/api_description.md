...
- `ability_model`：扫描、登记、查询和删除模型。删除本地文件时先把模型目录原子移动到模型根目录下的私有隔离区，再提交 SQLite 删除；进程若在两步之间退出，下次启动以 SQLite 为权威恢复未提交删除的目录，或清理已提交删除的隔离目录。未知隔离区内容会被保留并导致启动失败，避免误删运维人员数据。
