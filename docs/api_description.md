...
- `ability_download`：通过宿主机 Hugging Face CLI 下载、重试、取消、查询日志。Web Admin 从已登记 Hugging Face 模型发起下载并携带 `model_id`；CLI 退出成功后还会校验目标是模型根目录内真实可读的目录并计量文件大小，校验通过才将任务及同一模型记录转为 `succeeded` / `ready`，而不是产生与模型注册表脱节或无可用文件的假成功任务。
